...
void LED1_Task( void *pvParameters )
{
    ...
    /* Init timer */
    timer_set(&pLED1Test,100,CLOCK_TYPE_MS);
    timer_set(&pLED2Test,200,CLOCK_TYPE_MS);
	...
	{	
        ...
        if(timer_expired(&pLED1Test))
        {
            ...
            USART1_AppCall_SendString("[SYSTEM DEBUG]: Timerled1 expired, led 1 toggle \r\n");
        }
        if(timer_expired(&pLED2Test))
        {
            timer_restart(&pLED2Test);
            /* Toggle LED */
            LED_USER_2_TOGGLE;
            /* Print debug via UART1 */
            USART1_AppCall_SendString("[SYSTEM DEBUG]: Timerled2 expired, led 20 toggle \r\n");
        }
    }
}
#endif /* USE_OS */
